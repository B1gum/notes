\lecture{4}{September 3, 2025}{Control Volume Analysis II: Momentum \& Energy Equation}
...
We have previously defined Newton's second law for a system moving relative to an inertial coordinate system in \Cref{afs:N2} as:
\[ 
...
We have previously derived the relation between the system and control volume formulations in \Cref{eq:reytra} as:
\[ 
...
.\]
Since in deriving \Cref{eq:reytra} the system and control volume coincided t $t_0$, then
\[ 
...
When applying \Cref{eq:convolN2} we need to be a little careful. First we must choose a control volume and its control surface such that the volume integral and surface integral can be evaluated. In fluid mechanics the body force is usually gravity, so
\[ 
...
The momentum equation in \Cref{eq:convolN2} is a vector equation therefore it can be written in three scalar components as:
\begin{align*}
...
If we instead apply the approach to a differential control volume, we can obtain differential equations describing a flow field. Let us apply the continuity and momentum equations to a steady incompressible flow without friction as on \Cref{fig:f4_4}. The control volume is fixed in space and bounded by flow streamlines, and is thus an element of a stream tube. The length of the control volume is $\mathrm{d}s$. 
...
\begin{figure} [ht]
  \centering
  \includegraphics[width=0.8\linewidth]{./figures/f4_4.png}
...
To derive the control volume formulation of Newton's second law, we set $N = \textbf{P}_{xyz}$ and $\eta = \textbf{V}_{xyz}$. Substituting this into \Cref{eq:reytra} we obtain:
\[ 
...
\subsection{The Angular-Momentum Principle} \label{sec:angmomprin}
...
e = u + \frac{V^2}{2} + gz
.\]
We set $N = E$ and $\eta = e$ in \Cref{eq:reytra} and obtain:
\[ 
...
e = u + \frac{V^2}{2} + gz
.\]
Note that for steady flow the right hand side of \Cref{eq:convolT1} is zero.
...
\begin{figure} [ht]
  \centering
  \includegraphics[width=0.65\linewidth]{./figures/f4_6.png}
  \caption{Normal and shear stress forces.}
  \label{fig:f4_6}
\end{figure}

We can use this to compute the rate of work done by normal and shear stresses. We consider the segment og control surface shown on \Cref{fig:f4_6}. For an element of area $\mathrm{d}\textbf{A}$ we can write an expression for the normal stress force $\mathrm{d}\textbf{F}_{\mathrm{normal}}$. This will be given by the normal stress $\sigma_{nn}$ multiplied by the vector element $\mathrm{d}\textbf{A}$. Hence:
\[ 
...
As shown on \Cref{fig:f4_6} the shear force acting on an element of the control surface is given by:
\[ 
...
.\]
We set $N = S$ and $\eta = s$ in \Cref{eq:reytra} and obtain
\[ 
