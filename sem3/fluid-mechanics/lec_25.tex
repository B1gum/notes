...
We consider the propagation of a sound wave of infinitesimal strength into an undisturbed medium, shown on \Cref{fig:f12_1}a.
...
The flow of \Cref{fig:f12_1}a appears unsteady to a stationary observer, viewing the wave motion from a fixed point.
However, the flow appears steady to an observer located \textit{on} an inertial control volume moving with a segment of the wave as shown on \Cref{fig:f12_1}b.
...
The only surface acting in the $x$ direction on the control volume of \Cref{fig:f12_1}b are due to pressure.
...
\begin{figure} [ht]
  \centering
  \includegraphics[width=0.5\linewidth]{./figures/f12_2.png}
...
\begin{enumerate}
  \item $V = 0$.
    ...
    \Cref{fig:g12_2}a shows the conditions for the scenario after $3 \Delta t$ seconds.
  ...
  \item $0 < V < c$.
    ...
    \Cref{fig:f12_2}b shows the conditions for the scenario after $3 \Delta t$ seconds.
  ...
  \item $V = c$.
    ...
    \Cref{fig:f12_2}c shows conditions after $3\Delta t$ seconds.
    The source is shown at times $t = 0$, $\Delta t$, $2 \Delta t$, and $3 \Delta t$.
  ...
  \item $V>c$.
    ...
    \Cref{fig:f12_2}d shows the conditions after $3 \Delta t$ seconds.
    ...
    The region inside the cone is called the \textit{zone of action} and that outside the cone the \textit{zone of silence} as shown in \Cref{fig:f12_2}e.
    From geometry, we see from \Cref{fig:f12_2}d, that
    \begin{equation*}
...
or its equivalent
\begin{equation*}
  \frac{p_1}{\rho_1^{k}} = \frac{p_2}{\rho_2^{k}} = \frac{p}{\rho^{k}} = \text{constant}lii
.\end{equation*}
