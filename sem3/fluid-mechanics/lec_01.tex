\lecture{1}{August 25, 2025}{Introduction to Fluid Mechanics}
...
\rho = \rho(x,y,z,t)
\end{equation}
As density is a scalar quantity the field given by \Cref{eq:dens} is a scalar field. 

The density can also be expressed as the specific gravity, i.e. the weight compared to the maximum density of water, $\rho_{\mathrm{H}_2 \mathrm{O}} = \qty{1000}{\frac{kg}{m^3}} \text{ at } \qty{4}{\celsius} $. Thus the specific gravity of a substance can be found as:
\[ 
...
\end{equation}
As velocity is a vector quantity, the field given in \Cref{eq:velo} is a vector field.
...
A flow is either one-, two-, or three-dimensional depending on the amount of spatial coordinates required to specify the velocity field. \Cref{eq:velo} shows that the velocity field in some cases is a function of three spatial coordinates and time -- in this case it is a three-dimensional flow.
...
\begin{definition}[Uniform flow at a cross section]
  An assumption that states that a fluid has the same velocity everywhere in a given cross section as shown on \Cref{fig:f2_3}. This actually breaks the continuum hypothesis but it makes a lot of calculations easier.
\end{definition}
...
\end{equation}
Note that these are obtained at a given instant in time. If the flow is unsteady, time $t$ is held constant in \Cref{eq:stre}. Solution of the equation gives $y = y(x)$, with an undetermined integration constant, the value of which depends on the particular streamline.
...
For streaklines, the first step is to compute the pathline of a particle with \Cref{eq:path} that was released from the streak source at $x_0$, $y_0$ at time $t_0$, in the form
\[ 
...
\end{equation}
\Cref{eq:streak} gives the line generated (by time $t$) from a streak source placed at $(x_0, y_0)$. In these equations $t_0$ is varied from $0$ to $t$ to show the \textit{instantaneous} positions of all particles released up to time $t$. 
...
.\end{align*}
This is also shown graphically in \Cref{fig:f2_7}.

\begin{figure} [ht]
  \begin{subfigure}{0.48\linewidth}
    \centering
    \includegraphics[width=0.8\linewidth]{./figures/f2_7a.png}
    \caption{Force components}
  \end{subfigure}
  \begin{subfigure}{0.48\linewidth}
    \centering
    \includegraphics[width=0.8\linewidth]{./figures/f2_7b.png}
    \caption{Stress components}
  \end{subfigure}
...
\begin{figure} [ht]
  ...
  \centering
  \includegraphics[width=0.8\linewidth]{./figures/f2_9a.png}
  \caption{Fluid element at time $t$,}
  \end{subfigure}
  \begin{subfigure}{0.35\linewidth}
  \centering
  \includegraphics[width=0.8\linewidth]{./figures/f2_9b.png}
  \caption{Deformation of fluid element at time $t + \delta t$,}
  \label{fig:f2_9b}
  \end{subfigure}
  \hspace{0.2cm}
  \begin{subfigure}{0.35\linewidth}
  \centering
  \includegraphics[width=0.8\linewidth]{./figures/f2_9c.png}
  \caption{Deformation of fluid element at time $t + 2\delta t$.}
  \end{subfigure}
  \caption{Deformation of a fluid element.}
  \label{fig:f2_9}
\end{figure}

Focusing on the time interval $\delta t$ (\Cref{fig:f2_9b}) the deformation of the fluid is given by:
\[ 
...
Most common fluids are Newtonian under normal conditions. If the fluid on \Cref{fig:f2_9} is Newtonian, then:
\[ 
\tau_{yx} \propto \frac{\mathrm{d}u}{\mathrm{d}y} 
.\]
The constant of proportionality between these two quantities is the \textit{absolute viscosity} also called the dynamic viscosity, $\mu$. Thus in terms of the coordinates on \Cref{fig:f2_9} Newton's law of viscosity for one-dimensional flow is:
\begin{equation} \label{eq:newvisone}
  \tau_{yx} = \mu \frac{\mathrm{d}u}{\mathrm{d}y} 
\end{equation}
By dimensional analysis on \Cref{eq:newvisone} it can be seen that the units of viscosity are \unit{kg.\per.m.s} or \unit{Pa.s}.
...
\end{equation}
Here the term $\eta = k \left| \frac{\mathrm{d}u}{\mathrm{d}y}  \right|^{n-1}$ is referred to as the \textit{apparent viscosity}. When using \Cref{eq:nonnewvisone} we end up with a viscosity $\eta$ that is used in a formula in the same form as \Cref{eq:newvisone}. The big difference between these two is that while $\mu$ is constant (at a constant temperature), $\eta$ depends on the shear rate.
...
Droplets can either ``flatten out'' or remain as little drops when dropped on a surface. We define a liquid as \textit{wetting} a surface when the contact angle is $<\ang{90}$. This is shown on \Cref{fig:f2_11}.

\begin{figure} [ht]
  \begin{subfigure}{0.48\linewidth}
  \centering
  \includegraphics[width=0.6\linewidth]{./figures/f2_11a.png}
  \caption{A ``wetted'' surface}
  \end{subfigure}
  \begin{subfigure}{0.48\linewidth}
  \centering
  \includegraphics[width=0.6\linewidth]{./figures/f2_11b.png}
  \caption{A nonwetted surface}
  \end{subfigure}
